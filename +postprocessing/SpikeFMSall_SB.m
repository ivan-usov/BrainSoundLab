...
function [xdata, ydata, allx, ally, meanallx, meanally, stdrallx, stdrally] = SpikeFMSall_SB()
...
%         end
    ...
    function [x, x_label, y, y_label] = extractData(block1, block2, sitesDepth)
        ...
        column=[];
        for column=1:12
        Tempo_TRF(:,column) = cellfun(@(x) x(2,column), TRF); % 1-3: 40,60,80 dB; 1-12: -90 oct/s to +90 oct/s
        end
...
% % %     %---select only data with BF between 0 and 48 kHz
...
% % %         ind = (Thresh >= 0 & Thresh <= 80) ;
% % %     end

    %---select only data with block 1 type present
...
newname = strcat(userDir,'_AnalysisSpikesFMS60dB75up.mat');
...
analysis.BFnumber= y;
figname = strcat(userDir,'_AnalysisSpikesFMS60dB75up.fig');
