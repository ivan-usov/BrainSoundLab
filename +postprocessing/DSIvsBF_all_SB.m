...
function [xdata, ydata, allx,ally, meanallx, meanally, stdrallx, stdrally] = DSIvsBF_all_SB(temp_ind1, temp_ind2)
...
%         end
    ...
    function [x, x_label, y, y_label] = extractData(block1, block2, sitesDepth, ind1, ind2)
        BF = block1.custom.BF{1};
        DSI = block2.custom.DSIwospont{1};
        DSI = cellfun(@(x) x(ind1,ind2), DSI);   % ind1 is dB, ind2 is Rate
...
for k = 1:length(dir_ind)
    ...
    penNums = unique(penetration)';
        ...
        if length(block1_ind) == 1 && length(block2_ind) == 1
            ...
            [x, x_label, y, y_label] = extractData(mat1.block, mat2.block, sitesDepth, temp_ind1, temp_ind2);
...
all_dB=[5,10,20];
all_Rate=[15,30,45,60,75,90];
dB=all_dB(temp_ind1);
Rate=all_Rate(temp_ind2);

title([num2str(Rate),'oct/s @ ' ,num2str(dB), 'kHz']);
...
newname = strcat(userDir,'_Analysis',num2str(Rate),'oct',num2str(dB),'dB','.mat');
...
figname = strcat(userDir,'_Analysis',num2str(Rate),'oct',num2str(dB),'dB','.fig');
