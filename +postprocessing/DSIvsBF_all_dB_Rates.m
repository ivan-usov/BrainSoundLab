...
function [xdata, ydata] = DSIvsBF_all_dB_Rates(temp_ind1, temp_ind2)
...
%         end
    ...
    function [x, x_label, y, y_label] = extractData(block1, block2, ind1, ind2)
        BF = block1.custom.BF{1};
        DSI = block2.custom.DSIwospont{1};
        DSI = cellfun(@(x) x(ind1,ind2), DSI); % 2 - 60 dB, 6 - 90 oct/s
...
for k = 1:length(dir_ind)
    ...
    for penNum = penNums
        ...
        if length(block1_ind) == 1 && length(block2_ind) == 1
            ...
            [x, x_label, y, y_label] = extractData(mat1.block, mat2.block, temp_ind1, temp_ind2);
            xdata{k}{end+1} = x;
            ydata{k}{end+1} = y;
        else
...
xlabel(x_label)
ylabel(y_label)
title([num2str(temp_ind1), ', ', num2str(temp_ind2)]);
